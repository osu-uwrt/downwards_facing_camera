...
class CoralYolo {
   public:
    ...
    void LoadModel() {
        ...
        input = coral::MutableTensorData<float>(*interpreter->input_tensor(0))
                    .data();

        const TfLiteQuantizationParams& params = interpreter->output_tensor(0)->params;
        scale = params.scale;
        zeroPoint = params.zero_point;
    ...
    float postProcessValue(int8_t value) {
        return ((float) value - zeroPoint) / scale;
    }

    std::vector<Detection> detectImage(uint8_t* image) {
        preprocessImage(image);
        printf("Pixel 1 value %f %f %f\n", input[248520], input[248521],
               input[248522]); // At x: 280, y: 130 (Should be 205, 210, 213)
    ...
    std::vector<Detection> processBoxes(absl::Span<const int8_t> results) {
        ...
        for (int i = 0; i < 8400; i++) {
            ...
            // detection.bbox[3] = postProcessValue(results.at(i + 3 * 8400));
