...
static float iou(float *bbox1, float *bbox2) {
    ...
    */
    float xx = std::min<float>(bbox1[0] - (bbox1[2] / 2), bbox2[0] - (bbox2[2] / 2));
    float yy = std::max<float>(bbox1[1] - (bbox1[3] / 2), bbox2[1] - (bbox2[3] / 2));
    float aa = std::min<float>(bbox1[0] + (bbox1[2] / 2), bbox2[0] + (bbox1[2] / 2));
    float bb = std::min<float>(bbox1[1] + (bbox1[3] / 2), bbox2[1] + (bbox1[3] / 2));
...
public:
    ...
    CoralYolo(std::string model_path, int num_classes, float min_conf, float iou_thresh):
        model_path_(model_path), num_classes_(num_classes), min_conf_(min_conf), iou_thresh_(iou_thresh) {
        LoadModel();
        task_ = 1;
    }
    ...
    void LoadModel() {
        ...
        input = coral::MutableTensorData<int8_t>(*interpreter->input_tensor(0)).data();

        const TfLiteQuantizationParams &inputParams = interpreter->input_tensor(0)->params;
        inputScale = inputParams.scale;
        inputZeroPoint = inputParams.zero_point;

        const TfLiteQuantizationParams &bboxOutputParams = interpreter->output_tensor(0)->params;
        ...
        const TfLiteQuantizationParams &segOutputParams = interpreter->output_tensor(1)->params;
    ...
    void detectImage() override {
        ...
        memcpy(output0, coral::MutableTensorData<int8_t>(*interpreter->output_tensor(0)).data(),
               interpreter->output_tensor(0)->bytes);
        for (int i = 0; i < 204800; i++) {
            int8_t result = coral::MutableTensorData<int8_t>(*interpreter->output_tensor(1)).data()[i];
    ...
    std::vector<Detection> processDetections() {
        ...
        for (int i = 0; i < 2100; i++) {
            ...
            task_ = 0;
            if (task_ == 1) {
                partOfTask = detection.classId == bins[0];
            }
            else {
    ...
    void setTask(int task) { task_ = task; }
...
private:
    ...
    float inputZeroPoint, inputScale, bboxZeroPoint, bboxOutputScale, segZeroPoint, segOutputScale;
    ...
    static bool outputToBool(float value) { return (float) (1 / (1 + exp(-value))) > 0.5; }
    ...
    void processMask(Detection &detection, int loc) {
        ...
        for (int j = 0; j < 32; j++) {
            weights.data()[j] = postProcessValue(output0[loc + 4 + num_classes_ + j * 2100], true);
        }
    ...
    void nmsWithMask(std::vector<Detection> &detections, Detection &newDetection, float iouThreshold, int loc) {
        bool replaced = false;
...
std::shared_ptr<CoralYoloItf> createCoralYolo(std::string model_path, int num_classes, float min_conf,
                                              float iou_thresh) {
    return std::make_shared<CoralYolo>(model_path, num_classes, min_conf, iou_thresh);
}