#include "CanmoreImageReceiver.hpp"

#include "canmore/camera_feed_interface.h"
#include "canmore/crc32.h"
#include "canmore/protocol.h"

CanmoreImageReceiver::CanmoreImageReceiver(int ifIndex, uint8_t clientId, CanmoreImageReceiverHandler &handler):
...
void CanmoreImageReceiver::handleFrame(canid_t can_id, const std::span<const uint8_t> &data) {
...
void CanmoreImageReceiver::setStreamEnabled(bool enable) {
...
void CanmoreImageReceiver::setStreamId(uint8_t streamId) {
    canmore_camera_feed_cmd_t cmd = { .pkt = { .cmd = CANMORE_CAMERA_FEED_CMD_STREAM_ID,
                                               .data = { .stream_id = streamId } } };
    transmitFrame(CANMORE_CAMERA_FEED_CALC_CTRL_ID(clientId), cmd.data);
}

void CanmoreImageReceiver::setStreamQuality(uint8_t quality) {
